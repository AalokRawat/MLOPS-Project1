...
class DataProcessor:
    ...
    def balance_data(self, df):
        try :
            ...
            logger.error(f"Error during the data balancing {e}")
            raise CustomException("Error performing the data balancing", e)
        
    def select_features(self, df):
        try:
            logger.info("Starting the features selection")
            X = df.drop(columns='booking_status')
            y = df["booking_status"]
